...
export let knownTags = {
  '*': {
    attrs: 'accesskey, class, hidden?, id, lang, title, tabindex, style, aria-label, aria-labelledby, aria-describedby, onclick onkeypress onkeyup onkeydown onmousemove onmousewheel onmouseout onmouseenter itemscope? itemtype itemprop draggable'
  },
  '+': {}, // combinator
  '~': {}, // combinator
  a: {
  ...
  input: {
    ...
    attrs: {
      ...
      readonly: { type: 'boolean', alias: 'ro' }
    },
    otherAttrs: 'size checked? form formaction formenctype autocomplete autofocus autosave max maxlength min minlength multiple pattern spellcheck step width height disabled? onchange'
  },
  //j k
  label: {
    attrs: 'for'
