...
function atoa(x) { return Array.prototype.slice.call(x) }
...
function __decorate(f) {
  function ff(el,a,b,c) {
    if(typeof el == 'string') {
      ff(qsa(el),a,b,c) }
...
var on = __decorate(function(el,name,cb) {
  /*if (Array.isArray(el)) {
    el.forEach(function(el1){
      on(el1,name,cb)
    })
...
function onclick(el,cb) { return on(el,'click',cb) }

//function off(el,name,cb) {
var off = __decorate(function(el,name,cb) {
  /*if (Array.isArray(el)) {
    el.forEach(function(el1){
      off(el1,name,cb)
    })
